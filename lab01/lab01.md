...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - принимаемый энкодинг, принимаемые форматы, используемый браузер и версию браузера
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - Раздел "Line-based text data: text/html (10 lines)" под разделом "HTTP"
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да, Thu, 20 Feb 2025 06:59:02 GMT - время последней модификации файла полученного при первом GET запросе
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Загрузка выполнена последовательно, это следует из того, что значения поля "Time" записей с "No." =  492, 545 (соответствующие ответы) - различны.
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - http.cache_control, http.authorization, http.authbasic
