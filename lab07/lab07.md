...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   * `[SYN]` указано в поле `info` записи сегмента
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   * `relative sequence number` = `0` 
   * `acknowledgmed number` = 1
   * (`relative sequence number` + 1)
   * наличие флагов `[SYN]`, `[ACK]`
   ...
   * респонсы
   ...
   <table>
    ...
    <tr>
      ...
      <th>время получение ACK пака</th>
      <th>RTT</th>
    </tr>
  </thead>
  <tbody>
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   * `1-6_window_length` =  $740 + 5 \cdot 1474 \;(bytes) = 8110\;(bytes)$
   * `1-6_window` = $|8.161183 - 8.385045|\;(s) = 0.223862\;(s)$  
   |время отправки первого пакета - время получения ACK на последний пакет|  
   * Пропускная способность:
   $$\frac{8110\;(bytes)}{0.223862\;(s)} = 36227.676 \;(bytes/s)$$

   формула: $$\frac{\text{размер окна}}{\text{RTT для окна}} \;(bytes/s)$$
...
* работа трёх клиентов одновременно (один клиент фоном)
![a](images/s4a.png)

* прервали один клиент
![b](images/s4b.png)

* видим инфу о прерывании клиента от скрипта серва
![c](images/s4b.png)
